...
var daemonStopCmd = &cli.Command{
	Name:  "stop",
	Usage: "Stop a running shor daemon",
	Flags: []cli.Flag{},
...
var DaemonCmd = &cli.Command{
	Name:  "daemon",
	Usage: "Start a shor daemon process",
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "api",
			Value: "1234",
	...
	Action: func(cctx *cli.Context) error {
		...
		if cctx.String("genesis") != "" {
			...
			log.Infof("shor build.MaybeGenesis genBytes len:%d", len(genBytes))
		}
		...
		if cctx.Bool("remove-existing-chain") {
			...
			if !exists {
				return xerrors.Errorf("shor repo doesn't exist")
			}
		...
		if cctx.String(makeGenFlag) != "" {
			log.Infof("shor daemon makeGenFlag :", cctx.String(makeGenFlag))
