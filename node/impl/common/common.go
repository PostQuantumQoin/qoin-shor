...
func (a *CommonAPI) Version(ctx context.Context) (api.APIVersion, error) {
	v, err := api.VersionForType(api.RunningNodeType)
	if err != nil {
		return api.APIVersion{}, err
	}
	// nodeApi, _, err := cli.GetFullNodeAPIV1(ctx)
	// m := lotusminer.NewPqcMiner(nodeApi, "minerAddr", a.sf, a.j)

	// a.BlockMiner.Start(ctx)
