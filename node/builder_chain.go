...
import (
	...
	"github.com/filecoin-project/lotus/chain/wallet/pqcwallet"
...
var ChainNode = Options(
	...
	Override(new(wallet.Default), From(new(*pqcwallet.PqcWallet))),
	Override(new(api.Wallet), From(new(wallet.MultiWallet))),

	Override(new(*pqcwallet.PqcWallet), pqcwallet.NewWallet),
	// Override(new(*pqcwallet.PqcWallet), From(new(pqcwallet.PqcWallet))),
	...
	// Override(new(api.FullNode), From(new(impl.FullNodeAPI))),
	// Override(new(*miner.PqcMiner), modules.SetupBlockPqcProducer),
)
